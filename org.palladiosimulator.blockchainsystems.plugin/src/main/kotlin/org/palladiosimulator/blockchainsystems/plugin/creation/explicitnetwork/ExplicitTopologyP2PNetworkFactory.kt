...
import org.palladiosimulator.blockchainsystems.plugin.creation.LatencyValueProviderAdapter
import org.palladiosimulator.blockchainsystems.plugin.creation.ThroughputValueProviderAdapter
...
class ExplicitTopologyP2PNetworkFactory(private val topology: ExplicitNetworkTopology) : P2PNetworkFactory {
  override fun createP2PNetwork(): P2PNetworkCreationResult {
    ...
    for (designLink in topology.getLinks()) {
      ...
      val link = P2PLink(
        LatencyValueProviderAdapter.create(designLink.getSpecification().getLatencySpecification(), RandomGenerator.of("Random")),
        ThroughputValueProviderAdapter.create(designLink.getSpecification().getThroughputSpecification(), RandomGenerator.of("Random")),
        fromP2PNode,
        toP2PNode
      )
