...
public class EqualForkStubbornMiningNodeBehavior extends BlockchainNodeObject implements BlockchainSystemNodeBehavior {
    /**
     * Equal-Fork Stubborn Mining behavior.
     *
     * Difference to Lead-Stubborn Mining:
     * - When lead == 1 and an honest block appears, the attacker publishes
     *   exactly one private block to create a public fork (tie).
     */
    ...
    public void onBlockValidated(Block block, boolean isValid, BlockchainSystemNodeContext context) {
        ...
        // EQUAL FORK STUBBORN RELEASE RULES
        if (lead == 0) {
            // Behave honestly
            context.getBlockPropagationStrategy().distribute(block);
        } else if (lead == 1) {
            // DIFFERENCE to Lead-Stubborn : publish ONE block to create a public fork (tie)
    ...
    public Block onCreatingBlock(long blockMinedAt, String previousBlockHash, BlockchainSystemNodeContext context) {
        ...
        return context.getBlockFactory().createBlock(UUID.randomUUID().toString(), previousBlockHash, context.getId(), blockMinedAt, selection.getTotalSize(), selection.getTransactions());
    }
