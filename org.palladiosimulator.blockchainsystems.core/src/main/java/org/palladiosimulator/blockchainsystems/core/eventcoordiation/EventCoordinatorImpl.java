...
public class EventCoordinatorImpl implements EventCoordinator {
	...
	private static final Comparator<Event> EVENT_TIME_COMPARATOR = ((Event e1, Event e2) -> Long.compare(e1.getOccurenceTime(), e2.getOccurenceTime()));
	...
	private void purgeProcessedSlices() {
		...
		while (firstTimestamp != null && firstTimestamp.longValue() <= _clock.getCurrentTime()) {
			removeEventsAt(firstTimestamp.longValue());
	...
	private void processCurrentSlice() {
		long eventOccurenceTime = _clock.getCurrentTime();
		EffectsTimeSlice currentSlice = _eventTimeSlices.get(eventOccurenceTime);
	...
	public void raiseEvent(Event event) {
		if (event.getOccurenceTime() > _clock.getCurrentTime()) {
			scheduleEvent(event);
		} else if (event.getOccurenceTime() == _clock.getCurrentTime()) {
			dispatchEvent(event);
		}
	}

	private void scheduleEvent(Event event) {
		long eventOccurenceTime = event.getOccurenceTime();
		EffectsTimeSlice timeSlice = _eventTimeSlices.get(eventOccurenceTime);
		
		if (timeSlice == null) {
			timeSlice = new EffectsTimeSlice(eventOccurenceTime);
			_eventTimeSlices.put(eventOccurenceTime, timeSlice);
		}
	...
	public void cancelEventsFor(EventDispatchable eventOrigin) {
		...
		if (events != null && !events.isEmpty()) {
			for (Event event : events) {
				...
				EffectsTimeSlice timeSlice = _eventTimeSlices.get(event.getOccurenceTime());
