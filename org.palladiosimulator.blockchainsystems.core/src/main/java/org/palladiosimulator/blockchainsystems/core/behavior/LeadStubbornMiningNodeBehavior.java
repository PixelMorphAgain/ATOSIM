...
/**
 * Lead Stubborn Mining behavior.
 *
 * Difference to selfish mining:
 * - When lead == 1 and an honest block appears, the attacker does NOT immediately publish.
 * - The attacker keeps mining privately to try to regain the lead.
 */
public class LeadStubbornMiningNodeBehavior extends BlockchainNodeObject
    ...
    public void onBlockValidated(
        ...
        // ---- LEAD STUBBORN RELEASE RULES ----
        if (lead == 0) {
            // Behave honestly
            context.getBlockPropagationStrategy().distribute(block);
        }
        else if (lead == 1) {
            // DIFFERENCE: do NOTHING (keep mining privately)
            // No publication here
        }
        else {
